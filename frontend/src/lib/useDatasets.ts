...
const DATASET_MANIFESTS = ['datasets.json', 'hf_datasets.json'] as const;
...
function normalizeDataset(raw: unknown): Dataset | null {
  ...
  return {
    ...
    environment: firstString(raw.environment, raw.env, raw.image_environment),
...
async function fetchManifest(path: string): Promise<Dataset[]> {
...
export async function loadDatasets(): Promise<Dataset[]> {
  const results = await Promise.allSettled(DATASET_MANIFESTS.map((manifest) => fetchManifest(manifest)));
...
export function filterDatasets(
  ...
  if (opts.q?.trim()) {
    ...
    out = out.filter(
      (d) =>
        ...
        (typeof d.location === 'string' && d.location.toLowerCase().includes(q)) ||
        (Array.isArray(d.location) && d.location.some((country) => country.toLowerCase().includes(q))) ||
        (d.environment?.toLowerCase().includes(q)) ||
        (d.augmented_counterpart?.toLowerCase().includes(q)) ||
        (d.crop_types?.some((crop) => crop.toLowerCase().includes(q)) ?? false) ||
  ...
  for (const filter of opts.fieldFilters ?? []) {
    ...
    out = out.filter((d) => {
      ...
      if (filter.mode === 'containsAny' && Array.isArray(value)) {
        return value.some((entry) => filter.values!.includes(entry));
      }
